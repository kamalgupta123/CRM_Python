...
res = requests.get('http://103.107.67.94:50001/b1s/v1/Countries/$count', cookies=r.cookies, verify=False)
print(res.text)
...
for i in range(count):
    res = requests.get('http://103.107.67.94:50001/b1s/v1/Countries?$orderby=Code&$skip='+str(skip)+'', cookies=r.cookies, verify=False)
    opts = json.loads(res.text)
    print(len(opts['value']))

    for opt in opts['value']:
        print(opt['Code'])
        opp_sql = "INSERT INTO `Countries_countries` (`Code`, `Name`) VALUES ('"+str(opt['Code'])+"', '"+str(opt['Name'])+"');"
