...
def top5itembyamount(request):
    ...
    top2bp = InvoiceDocumentLines.objects.raw('SELECT id, ItemCode, ItemDescription, sum(Quantity*UnitPrice) as Total FROM `Invoice_documentlines` group by ItemCode order by Total desc limit 5;')
    top5=[]

    for od in top2bp:
        print(od.ItemCode)
        print(od.ItemDescription)
        print(od.Total)
        top5.append({"ItemCode":od.ItemCode, "ItemName":od.ItemDescription, 'Total':od.Total})
...
def top5bp(request):
    ...
    try:
        top2bp = Order.objects.raw('SELECT id,CardCode, sum(DocTotal) as Total FROM `Invoice_invoice` group by CardCode order by Total desc limit 5;')
        top5=[]
        for od in top2bp:
            try:
                ...
                print(cd.CardName)
                print(od.Total)
                top5.append({"CardCode":cd.CardCode, "CardName":cd.CardName, 'Total':od.Total})
